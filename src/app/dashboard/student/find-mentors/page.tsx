    ...
    return (
      <div className="min-h-screen bg-card flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-foreground/80">Loading mentors...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex">
        ...
        <main className="flex-1 p-4 md:p-8 cl-main">
          <div className="max-w-6xl mx-auto">
            {/* Welcome Banner */}
            <div className="rounded-xl p-6 md:p-8 mb-6 border border-accent-purple bg-accent-purple/10">
              <div className="flex items-center justify-between gap-6">
                <div className="flex-1">
                  ...
                  <p className="text-foreground/80 text-sm md:text-base leading-relaxed max-w-2xl">
                    Connect with experienced mentors from <span className="font-semibold text-accent-purple">{profile.universities?.name}</span> who can guide you in your learning journey
                  </p>
                  <div className="flex items-center gap-4 mt-4">
                    <div className="px-4 py-2 bg-[rgba(255,255,255,0.6)] backdrop-blur-sm rounded-lg border border-accent-purple">
                      <span className="text-2xl font-semibold text-accent-purple">{filteredMentors?.length || 0}</span>
                      <span className="text-xs text-foreground/80 ml-2">{searchQuery || selectedExpertise !== 'All Expertise' ? 'Filtered' : 'Available'} Mentors</span>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-6">
              ...
              <div className="relative">
                <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-accent-purple pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <svg className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground/70 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {filteredMentors && filteredMentors.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredMentors.map((mentor, index) => {
                  const cardColors = [
                    { top: 'bg-amber-500/10', badge: 'bg-amber-500/10 border-amber-500 text-amber-600', accent: '#FB923C' },
                    { top: 'bg-green-500/10', badge: 'bg-green-500/10 border-green-600 text-green-600', accent: '#34D399' },
                    { top: 'bg-accent-purple/10', badge: 'bg-accent-purple/10 border-accent-purple text-accent-purple', accent: '#A78BFA' },
                    { top: 'bg-accent-purple/10', badge: 'bg-accent-purple/10 border-accent-purple text-accent-purple', accent: '#60A5FA' },
                    { top: 'bg-accent-purple/10', badge: 'bg-accent-purple/10 border-accent-purple text-accent-purple', accent: '#F472B6' },
                    { top: 'bg-muted/40', badge: 'bg-muted border-border text-foreground/80', accent: '#9CA3AF' },
                  ];
                  ...
                  return (
                    <div
                      key={mentor.id}
                      className="w-full rounded-xl border border-black/5 transition-transform hover:scale-[1.02] overflow-hidden"
                    >
                      ...
                      <div className={`${colorScheme.top} p-5 relative`}>
                        ...
                        <div className="flex items-center justify-between mb-4 relative z-10">
                          {mentor.avatar_url ? (
                            ...
                            <div className="w-16 h-16 rounded-xl flex items-center justify-center text-white text-2xl font-semibold bg-primary">
                        ...
                        {/* Name and Expertise */}
                        <h3 className="text-xl font-semibold leading-snug mb-3 text-foreground line-clamp-1 min-h-[1.75rem] relative z-10">
                          {mentor.full_name}
                        </h3>
                        ...
                        <div className="flex flex-wrap gap-2 min-h-[2.5rem] relative z-10">
                          <span className="px-3 py-1 rounded-full text-xs bg-card border border-black/10 text-foreground/80 flex items-center gap-1">
                          ...
                          {mentor.specialization && (
                            <span className="px-3 py-1 rounded-full text-xs bg-card border border-black/10 text-foreground/80">
                              {mentor.specialization}
                            </span>
                          )}
                        </div>
                        ...
                        <div className="flex items-center justify-between relative z-10">
                          ...
                          {(() => {
                            ...
                            if (connection?.status === 'accepted' || hasActiveChat) {
                              return (
                                <Link
                                  ...
                                  className="ml-3 text-white px-5 py-2.5 rounded-full text-sm font-semibold transition-all whitespace-nowrap flex items-center gap-2 bg-primary"
                                >
                            ...
                            if (connection?.status === 'pending') {
                              return (
                                <div className="ml-3 bg-muted text-muted-foreground px-5 py-2.5 rounded-full text-sm font-semibold border border-border whitespace-nowrap flex items-center gap-2">
                            ...
                            return (
                              <Link
                                ...
                                className="ml-3 bg-black text-white px-5 py-2.5 rounded-full text-sm font-medium hover:bg-neutral-900 transition-colors whitespace-nowrap"
                              >
                                Connect
                              </Link>
                            );
              ...
              <div className="rounded-xl p-12 text-center border border-border bg-background">
                <div className="w-20 h-20 rounded-xl flex items-center justify-center mx-auto mb-4 bg-accent-purple/10">
                ...
                {(searchQuery || selectedExpertise !== 'All Expertise') && (
                  <button
                    ...
                    className="px-6 py-2.5 bg-primary hover:bg-primary text-white rounded-lg font-medium transition-all"
                  >
                    Clear Filters
                  </button>
                )}
