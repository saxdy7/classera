...
export default function ComparisonDashboard({ assignmentId, maxScore, submissions }: ComparisonDashboardProps) {
  ...
  const ScoreBadge = ({ score }: { score: number | undefined }) => {
    if (score === undefined) return <span className="text-muted-foreground/70">—</span>;
    const color = score >= 80 ? 'text-green-600 bg-green-500/10' : score >= 60 ? 'text-amber-600 bg-amber-500/10' : 'text-destructive bg-destructive/10';
    ...
    <div className="space-y-6">
      ...
      {withAnalytics.length > 0 && (
        <div className="grid sm:grid-cols-2 gap-4">
          {topPerformer && (
            <div className="flex items-center gap-3 bg-amber-500/10 border border-amber-500 rounded-xl p-4">
              ...
              <div>
                ...
                <p className="text-sm font-semibold text-foreground">{topPerformer.student_name}</p>
                <p className="text-xs text-muted-foreground">Overall score: {topPerformer.analytics!.overall_score}</p>
              </div>
            </div>
          )}
          {needsAttention && (
            <div className="flex items-center gap-3 bg-destructive/10 border border-destructive rounded-xl p-4">
              <div className="w-10 h-10 bg-destructive/10 rounded-full flex items-center justify-center flex-shrink-0">
                <AlertTriangle size={18} className="text-destructive" />
              </div>
              <div>
                <p className="text-xs text-destructive font-semibold uppercase tracking-wide">Needs Attention</p>
                <p className="text-sm font-semibold text-foreground">{needsAttention.student_name}</p>
                <p className="text-xs text-muted-foreground">Overall score: {needsAttention.analytics!.overall_score}</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          <h3 className="text-sm font-semibold text-foreground/80 mb-4">Score Distribution</h3>
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          <h3 className="text-sm font-semibold text-foreground/80 mb-1">Performance Radar</h3>
          <p className="text-xs text-muted-foreground/70 mb-3">Select up to 5 students from the table below</p>
          {selectedIds.length === 0 ? (
            <div className="flex items-center justify-center h-[180px] text-muted-foreground/70 text-sm">
            ...
            <tbody className="divide-y divide-border">
              {sorted.map((sub) => {
                ...
                return (
                  <tr
                    key={sub.id}
                    className={`hover:bg-slate-50/60 transition-colors ${isSelected ? 'bg-card' : ''}`}
                  >
                    <td className="pl-3">
                      <button
                    ...
                    <td className="px-3 py-2.5">
                      <p className="font-medium text-foreground text-sm">{sub.student_name}</p>
                      <p className="text-xs text-muted-foreground/70">{sub.student_email}</p>
                    </td>
                    ...
                    <td className="px-3 py-2.5 text-center text-foreground/80 font-mono text-xs">{ana?.total_commits ?? '—'}</td>
                    <td className="px-3 py-2.5 text-center">
                      {sub.score !== undefined
                        ? <span className="font-semibold text-foreground/80">{sub.score}<span className="text-muted-foreground/70 font-normal">/{maxScore}</span></span>
                    ...
                    <td className="px-3 py-2.5">
                      <Link
                        ...
                        className="flex items-center gap-1 text-xs text-accent-purple hover:underline whitespace-nowrap"
                      >
...
function SortTH({
  ...
  return (
    <th
      ...
      onClick={() => onSort(col)}
    >
      <span className={`flex items-center gap-1 text-xs font-medium ${active ? 'text-accent-purple' : 'text-muted-foreground'}`}>
        {label}
