        ...
        <div className="space-y-8">
            ...
            <div className="bg-card rounded-xl p-4 md:p-6 border border-border">
                    ...
                    <div className="flex flex-col lg:flex-row gap-3">
                        ...
                        <div className="flex gap-2 flex-shrink-0">
                            ...
                            <button
                                ...
                                className="px-4 md:px-5 py-3 rounded-lg font-medium transition-all flex items-center gap-2 whitespace-nowrap text-white hover:opacity-90 disabled:opacity-50 bg-green-600"
                            >
                                {liveSearching ? (
                                    <>
                            ...
                            <button
                                ...
                                className={`px-4 md:px-6 py-3 rounded-lg font-medium transition-all flex items-center gap-2 whitespace-nowrap ${showFilters ? 'bg-primary text-white' : 'bg-muted hover:bg-muted text-foreground/80'
                                    }`}
                            >
                            ...
                            <button
                                ...
                                className="w-12 h-12 bg-muted hover:bg-muted rounded-lg transition-colors flex items-center justify-center text-lg"
                    ...
                    {showFilters && (
                        <div className="pt-4 border-t border-border">
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                {/* Type Filter */}
                                <div>
                                    <label className="block text-xs font-semibold text-foreground/80 mb-1.5 uppercase tracking-wide">Type</label>
                                    <select
                                        value={selectedType}
                                ...
                                {/* Platform Filter */}
                                <div>
                                    <label className="block text-xs font-semibold text-foreground/80 mb-1.5 uppercase tracking-wide">Platform</label>
                                    <select
                                ...
                                {/* Level Filter */}
                                <div>
                                    <label className="block text-xs font-semibold text-foreground/80 mb-1.5 uppercase tracking-wide">Level</label>
                                    <select
                                        value={selectedLevel}
                                ...
                                {/* Sort By */}
                                <div>
                                    <label className="block text-xs font-semibold text-foreground/80 mb-1.5 uppercase tracking-wide">Sort By</label>
                                    <select
                                        value={sortBy}
                            ...
                            {(selectedType !== 'all' || selectedPlatform !== 'all' || selectedLevel !== 'all') && (
                                <button
                                    ...
                                    className="mt-3 px-4 py-2 text-sm text-accent-purple hover:text-accent-purple hover:bg-accent-purple/10 rounded-lg transition-colors"
                                >
                                    ✕ Clear all filters
                                </button>
                            )}
                ...
                {(searchQuery || selectedType !== 'all' || selectedPlatform !== 'all' || selectedLevel !== 'all') && (
                    <div className="mt-4 flex flex-wrap gap-2">
                        {searchQuery && (
                            <span className="px-3 py-1 bg-accent-purple/10 text-accent-purple rounded-full text-sm flex items-center gap-2">
                                Search: "{searchQuery}"
                        ...
                        {selectedLevel !== 'all' && (
                            <span className="px-3 py-1 bg-green-500/10 text-green-600 rounded-full text-sm flex items-center gap-2">
                                {selectedLevel}
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                    <p className="text-foreground/80">{filteredCourses.length} courses</p>
                    {favorites.size > 0 && (
                        <span className="px-3 py-1 bg-destructive/10 text-destructive rounded-full text-sm font-semibold flex items-center gap-1">
            ...
            {loading ? (
                ...
                <div className="bg-card rounded-lg p-12 border border-border text-center">
                    ...
                    <p className="text-foreground/80 mb-4">Try adjusting your search or filters</p>
                    <button
                        onClick={() => {
                            setSearchQuery('');
                            setSelectedType('all');
                ...
                <div className={viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6' : 'space-y-4'}>
                    {filteredCourses.map((course) => (
                        <div
                            key={course.id}
                            className={`bg-card rounded-lg border border-border overflow-hidden transition-all group ${viewMode === 'list' ? 'flex' : ''
                                }`}
                        >
                            {/* Course Image */}
                            <div className={`relative overflow-hidden ${viewMode === 'list' ? 'w-64' : 'h-48'}`}>
                                ...
                                <div className="absolute top-3 right-3 flex gap-2">
                                    <span className={`px-3 py-1 rounded-full text-xs font-semibold backdrop-blur-sm ${course.type === 'free'
                                        ? 'bg-[rgba(22,163,74,0.9)] text-white'
                                        : 'bg-[rgba(13,116,206,0.9)] text-white'
                                        }`}>
                                ...
                                >
                                    <Heart className={`w-4 h-4 ${favorites.has(course.id) ? 'fill-destructive text-destructive' : 'text-foreground/80'}`} />
                                </button>
                            </div>
                            ...
                            <div className="p-6 flex-1">
                                ...
                                <div className="flex items-center gap-2 mb-3 flex-wrap">
                                    <span className="text-xs font-semibold text-accent-purple bg-accent-purple/10 px-2 py-1 rounded">
                                        {course.platform}
                                    </span>
                                    ...
                                    {course.rating >= 4.5 && (
                                        <span className="text-xs font-semibold text-amber-600 bg-amber-500/10 px-2 py-1 rounded flex items-center gap-1">
                                ...
                                {/* Title */}
                                <h3 className="font-semibold text-foreground mb-2 line-clamp-2 group-hover:text-accent-purple transition-colors">
                                    {course.title}
                                </h3>

                                {/* Instructor */}
                                <p className="text-sm text-foreground/80 mb-3">{course.instructor}</p>
                                ...
                                <div className="flex flex-wrap gap-2 mb-4">
                                    {course.skills.slice(0, 3).map((skill, idx) => (
                                        <span key={idx} className="text-xs bg-muted text-foreground/80 px-2 py-1 rounded">
                                            {skill}
                                        </span>
                                    ))}
                                    {course.skills.length > 3 && (
                                        <span className="text-xs text-muted-foreground">+{course.skills.length - 3} more</span>
                                    )}
                                </div>
                                ...
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        className={`flex-1 px-4 py-2 rounded-lg font-medium transition-all flex items-center justify-center gap-2 ${enrolled.has(course.id)
                                            ...
                                            : 'text-white hover:opacity-90 bg-primary'
                                            }`}
                                    >
                                    ...
                                    <a
                                        ...
                                        className="px-4 py-2 bg-muted hover:bg-muted rounded-lg transition-colors flex items-center justify-center"
            ...
            <div className="pt-8 border-t border-border">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-2xl font-semibold text-foreground">YouTube Tutorials</h2>
                            <p className="text-sm text-foreground/80">Educational content only • Filtered for quality</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {videos.map((video) => (
                            <div
                                ...
                                className="bg-card rounded-lg border border-border overflow-hidden transition-all cursor-pointer group"
                            >
                                {/* Video Thumbnail */}
                                ...
                                {/* Video Info */}
                                <div className="p-4">
                                    <h3 className="font-semibold text-foreground mb-2 line-clamp-2 group-hover:text-destructive transition-colors">
                                        {video.title}
                                    </h3>
                                    <p className="text-sm text-foreground/80 mb-1 font-medium">{video.channel}</p>
            ...
            {selectedVideo && (
                <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setSelectedVideo(null)}>
                    <div className="relative w-full max-w-5xl bg-black rounded-xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
                        <button
