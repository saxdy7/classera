        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              {filteredRoadmaps.length > 0 ? (
                <div className="space-y-3">
                  {filteredRoadmaps.map(roadmap => (
                    <div key={roadmap.id} className="p-4 border border-border rounded-lg hover:border-accent-purple transition">
                      ...
                      <p className="text-sm text-foreground/80 mt-1">{roadmap.progress || 0}% complete</p>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-muted-foreground py-4 text-center">No learning paths match your search</p>
              )}
            </div>
          )}
              ...
              {filteredCourses.length > 0 ? (
                <div className="space-y-3">
                  {filteredCourses.map((enrollment, idx) => (
                    <div key={idx} className="p-4 border border-border rounded-lg hover:border-accent-purple transition">
                      <p className="font-medium text-black">{enrollment.title || 'Course'}</p>
                      <p className="text-sm text-foreground/80">{enrollment.subject || 'General'}</p>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-muted-foreground py-4 text-center">No courses match your search</p>
              )}
            </div>
          )}
          ...
          {searchQuery && filteredRoadmaps.length === 0 && filteredCourses.length === 0 && (
            <div className="bg-card rounded-lg p-12 text-center">
              <Search className="w-12 h-12 text-muted-foreground/70 mx-auto mb-3" />
          ...
          <div className="bg-card rounded-lg p-6 h-fit">
            ...
            <div className="space-y-4">
              ...
              {githubConn.public_repos && (
                <div className="p-4 bg-muted/40 rounded-lg">
                  <p className="text-sm text-foreground/80">Public Repositories</p>
                  <p className="text-2xl font-semibold text-black">{githubConn.public_repos}</p>
                </div>
              )}
              {githubConn.followers && (
                <div className="p-4 bg-muted/40 rounded-lg">
                  <p className="text-sm text-foreground/80">Followers</p>
