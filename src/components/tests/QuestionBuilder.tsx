...
export function QuestionBuilder({ questions, onChange, testSubject }: QuestionBuilderProps) {
    ...
    const getTypeColor = (type: string) => {
        switch (type) {
            case 'mcq': return 'bg-[rgba(13,116,206,0.12)] text-[var(--cl-info)]';
            case 'descriptive': return 'bg-[var(--cl-primary-soft)] text-[var(--cl-primary)]';
            case 'short_answer': return 'bg-[rgba(22,163,74,0.12)] text-[var(--cl-success)]';
          ...
          <div className="lg:col-span-3 space-y-6">
            {questions.map((question, qIndex) => (
                <div
                    ...
                    className={`bg-[var(--cl-surface-card)] rounded-[var(--cl-r-lg)] p-6 border border-[var(--cl-hairline)] ${draggedIndex === qIndex ? 'opacity-50' : ''}`}
                >
                    {/* Question Header */}
                    <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-3">
                            <div className="cursor-grab text-[var(--cl-muted-soft)] hover:text-[var(--cl-body)]">
                    ...
                    {question.type === 'mcq' && (
                        <div className="mb-4">
                                    ...
                                    <div key={oIndex} className="flex items-center gap-3">
                                        ...
                                        {(question.options?.length || 0) > 2 && (
                                            <button
                                                ...
                                                className="p-2 text-[var(--cl-muted-soft)] hover:text-[var(--cl-error)] transition-colors"
                                            >
                            ...
                            <button
                                ...
                                className="mt-2 text-sm text-[var(--cl-primary)] hover:text-[var(--cl-primary)] font-medium"
                            >
                                + Add Option
                            </button>
            ...
            <div className="grid grid-cols-3 gap-4">
                <button
                    ...
                    className="py-4 border-2 border-dashed border-[var(--cl-hairline-strong)] rounded-[var(--cl-r-lg)] hover:border-[var(--cl-info)] hover:bg-[rgba(13,116,206,0.12)] transition-all flex flex-col items-center justify-center gap-2 text-[var(--cl-body)] hover:text-[var(--cl-info)] font-medium"
                >
                ...
                <button
                    ...
                    className="py-4 border-2 border-dashed border-[var(--cl-hairline-strong)] rounded-[var(--cl-r-lg)] hover:border-[var(--cl-primary)] hover:bg-[var(--cl-primary-soft)] transition-all flex flex-col items-center justify-center gap-2 text-[var(--cl-body)] hover:text-[var(--cl-primary)] font-medium"
                >
                ...
                <button
                    ...
                    className="py-4 border-2 border-dashed border-[var(--cl-hairline-strong)] rounded-[var(--cl-r-lg)] hover:border-[var(--cl-success)] hover:bg-[rgba(22,163,74,0.12)] transition-all flex flex-col items-center justify-center gap-2 text-[var(--cl-body)] hover:text-[var(--cl-success)] font-medium"
                >
            ...
            <div className="lg:sticky lg:top-6">
              <div className="bg-[var(--cl-surface-card)] rounded-[var(--cl-r-xl)] border border-[var(--cl-hairline)] p-6">
                <div className="flex items-center gap-2 mb-1">
                  <Eye className="w-4 h-4 text-[var(--cl-primary)]" />
                  <h3 className="text-sm font-semibold text-[var(--cl-ink)] uppercase tracking-wide">Live Preview</h3>
                </div>
                <p className="text-xs text-[var(--cl-muted)] mb-5">How students will see your test</p>

                {questions.length === 0 ? (
                  <div className="text-center py-12 text-[var(--cl-muted-soft)] text-sm border border-dashed border-[var(--cl-hairline)] rounded-[var(--cl-r-lg)]">
                    Add a question to see the preview
                  </div>
                ) : (
                  <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-1">
                    {questions.map((q, i) => (
                      <div key={q.id} className="pb-5 border-b border-[var(--cl-hairline)] last:border-0 last:pb-0">
                        <div className="flex items-start gap-2 mb-3">
                          <span className="flex-shrink-0 w-6 h-6 rounded-lg bg-[var(--cl-primary-soft)] text-[var(--cl-primary)] text-xs font-semibold flex items-center justify-center">
                            {i + 1}
                          </span>
                          <p className="text-sm font-semibold text-[var(--cl-ink)] leading-snug">
                            {q.question || <span className="text-[var(--cl-muted-soft)] italic font-normal">Untitled question</span>}
                          </p>
                        </div>
                        ...
                        {q.type === 'mcq' ? (
                          <div className="space-y-2 pl-8">
                            {(q.options || []).map((opt, oi) => {
                              ...
                              return (
                                <div
                                  key={oi}
                                  className={`flex items-center gap-3 px-4 py-2.5 rounded-[var(--cl-r-lg)] border text-sm transition-colors ${
                                    isCorrect ? 'border-[var(--cl-success)] bg-[rgba(22,163,74,0.12)]' : 'border-[var(--cl-hairline)]'
                                  }`}
                                >
                                  <span
                                    className={`w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${
                                      isCorrect ? 'border-[var(--cl-success)] bg-[var(--cl-success)]' : 'border-[var(--cl-hairline-strong)]'
                                    }`}
                                  >
                                    {isCorrect && <Check className="w-2.5 h-2.5 text-[var(--cl-on-dark)]" />}
                                  </span>
                                  <span className={isCorrect ? 'text-[var(--cl-success)] font-medium' : 'text-[var(--cl-body)]'}>
                                    {opt || `Option ${oi + 1}`}
                                  </span>
                                </div>
                              );
                          ...
                          <div className="pl-8">
                            <div className="w-full px-4 py-3 rounded-[var(--cl-r-lg)] border border-dashed border-[var(--cl-hairline)] text-sm text-[var(--cl-muted-soft)]">
                        ...
                        <div className="pl-8 mt-2">
                          <span className="text-[11px] font-medium text-[var(--cl-muted-soft)]">
