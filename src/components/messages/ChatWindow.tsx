...
export function ChatWindow({ currentUserId }: { currentUserId: string }) {
    ...
    return (
        <div className="flex flex-col h-full bg-[rgba(250,250,247,0.5)]">
            ...
            {/* Header */}
            <div className="h-20 border-b border-border bg-card px-6 flex items-center justify-between flex-shrink-0">
                <div className="flex items-center gap-4">
                    <button
                        onClick={() => setActiveConversation(null)}
                        className="lg:hidden p-2 -ml-2 hover:bg-muted rounded-lg text-foreground/80 transition-colors"
                    >
                        <ArrowLeft className="w-5 h-5" />
                    </button>
                    <div className="relative flex-shrink-0">
                        <div className="w-11 h-11 rounded-full bg-accent-purple/10 flex items-center justify-center text-accent-purple font-semibold text-lg ring-2 ring-white">
                        ...
                        {isOnline && (
                            <span className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-green-600 border-2 border-white rounded-full animate-pulse"></span>
                        )}
                    </div>
                ...
                <div className="flex items-center gap-1">
                    <button className="p-2.5 text-muted-foreground/70 hover:text-accent-purple hover:bg-accent-purple/10 rounded-lg transition-all">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4" ref={scrollRef}>
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-center px-4">
                        <div className="w-20 h-20 rounded-full flex items-center justify-center mb-4 ring-8 ring-border bg-accent-purple/10">
                            <MessageSquare className="w-9 h-9 text-accent-purple" />
                        </div>
                        <p className="text-foreground/80 font-semibold text-lg mb-1">No messages yet</p>
                        <p className="text-sm text-muted-foreground/70 max-w-xs">Start the conversation by sending a message below</p>
                    </div>
                ) : (
                    <>
                                ...
                                <div key={msg.id} className={`flex gap-3 ${isMe ? 'flex-row-reverse' : ''}`}>
                                    {!isMe && (
                                        <div className="w-8 flex-shrink-0 flex flex-col justify-end">
                                            {showAvatar ? (
                                                <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold text-accent-purple ring-2 ring-white bg-accent-purple/10">
                                                    {otherUser.full_name[0]}
                                                </div>
                                            ) : <div className="w-8" />}
                                        </div>
                                    ...
                                    <div className={`flex flex-col max-w-[70%] sm:max-w-[65%] ${isMe ? 'items-end' : 'items-start'}`}>
                                        <div className={`px-4 py-2.5 rounded-xl text-[15px] leading-relaxed relative group ${isMe
                                            ...
                                            {msg.type === 'file' && msg.file_url && (
                                                <a
                                                    ...
                                                    className={`flex items-center gap-3 p-3 rounded-lg mb-2 transition-colors ${isMe ? 'bg-[rgba(255,255,255,0.1)] hover:bg-[rgba(255,255,255,0.2)]' : 'bg-muted/40 hover:bg-muted'
                                                        }`}
                                                >
                                                    <div className={`w-10 h-10 ${isMe ? 'bg-[rgba(255,255,255,0.2)]' : 'bg-accent-purple/10'} rounded-lg flex items-center justify-center flex-shrink-0`}>
                                                        <Paperclip className={`w-5 h-5 ${isMe ? 'text-white' : 'text-accent-purple'}`} />
                                                    </div>
                                                    <div className="flex-1 min-w-0">
                                                        <p className="font-medium truncate text-sm">{msg.file_name || 'Attached File'}</p>
                                                        <p className={`text-xs ${isMe ? 'text-accent-purple' : 'text-muted-foreground'}`}>
                                            ...
                                            <div className={`flex items-center gap-1.5 absolute bottom-1.5 right-3 opacity-0 group-hover:opacity-100 transition-opacity`}>
                                                <span className={`text-[10px] ${isMe ? 'text-accent-purple' : 'text-muted-foreground/70'}`}>
                                                    {format(new Date(msg.created_at), 'h:mm a')}
                                                </span>
                                                {isMe && (
                                                    msg.read_at ? (
                                                        <CheckCheck className="w-3.5 h-3.5 text-accent-purple" />
                                                    ) : (
                        ...
                        {isOtherUserTyping && (
                            <div className="flex gap-3">
                                ...
                                <div className="px-5 py-3 rounded-xl rounded-bl-md bg-card border border-border">
